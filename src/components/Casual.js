...
const CasualPage = () => {
  ...
  useEffect(() => {
    const fetchCasual = async () => {
      try {
        setLoading(true);
        // Note: Using category ID 2 for Casual shoes
        const response = await axios.get("http://localhost:5001/api/v1/store/categories/2/all-products");
        setProducts(response.data.products);
        setError(null);
      } catch (err) {
        console.error("Error fetching casual shoes:", err);
        setError("Failed to load casual shoes");
      } finally {
        setLoading(false);
      }
    };
    ...
    <>
      <div className="product-container">
        ...
        <div className="filter-bar">
          ...
          <input
            type="text"
            placeholder="Search casual shoes..."
            value={searchQuery}
        ...
        <div className="product-grid">
          {sortedProducts.length === 0 ? (
            <div className="no-products">No casual shoes found</div>
          ) : (
