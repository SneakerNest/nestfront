...
const BootsPage = () => {
  ...
  useEffect(() => {
    const fetchBoots = async () => {
      try {
        setLoading(true);
        // Using category ID 3 for Boots
        const response = await axios.get("http://localhost:5001/api/v1/store/categories/3/all-products");
        setProducts(response.data.products);
        setError(null);
      } catch (err) {
        console.error("Error fetching boots:", err);
        setError("Failed to load boots");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      <div className="product-container">
        <h1 className="category-title">Boots</h1>
        ...
        <div className="product-grid">
          {sortedProducts.length === 0 ? (
            <div className="no-products">No boots found</div>
          ) : (
