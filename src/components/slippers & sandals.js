// src/components/SlippersSandalsPage.js
...
const SlippersSandalsPage = () => {
  ...
  useEffect(() => {
    const fetchSlippersSandals = async () => {
      try {
        setLoading(true);
        // Using category ID 4 for Slippers & Sandals
        const response = await axios.get("http://localhost:5001/api/v1/store/categories/4/all-products");
        setProducts(response.data.products);
        setError(null);
      } catch (err) {
        console.error("Error fetching slippers & sandals:", err);
        setError("Failed to load slippers & sandals");
      } finally {
        setLoading(false);
      }
    };

    fetchSlippersSandals();
  }, []);
  ...
  return (
    <>
      <div className="product-container">
        <h1 className="category-title">Slippers & Sandals</h1>
        ...
        <div className="filter-bar">
          ...
          <input
            type="text"
            placeholder="Search slippers & sandals..."
            value={searchQuery}
        ...
        <div className="product-grid">
          {sortedProducts.length === 0 ? (
            <div className="no-products">No slippers or sandals found</div>
          ) : (
...
export default SlippersSandalsPage;
